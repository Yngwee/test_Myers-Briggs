...
if __name__ == '__main__':
    ...
    print(f'Буквенный код оценки: {user.user_nnnn},  Вашей оценкой является балл: {user.user_score["user_ie"]} '
    ...
    print('Помните, что темпераменты и типы - это возможности, а не способности и что существует корреляция. '
          ...
          'способности в этом виде деятельности. Тогда вероятно, что предыдущий параграф верен, хотя исключения всегда '
          'найдутся. Так что NF может иметь в себе достаточно SР, чтобы быть превосходным в стратегии и лучшим в логистике, чем можно было бы ожидать (хотя логистика бы должна была быть его наименее квалифицированной ролью).')
